...
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
					char const* streamName, char const* fileName); // forward

ServerMediaSession* DynamicRTSPServer
::lookupServerMediaSession(char const* streamName, Boolean isFirstLookupInSession)
{
  std::string stream_name(streamName);
  std::unordered_map<std::string, std::string>::const_iterator it;

  for (it = fStreamDirMap.begin(); it != fStreamDirMap.end(); it++) {
    std::string path = (*it).first;
    std::string loc = (*it).second;
    if (stream_name.compare(0, path.size(), path) == 0) {
      std::string fullName = loc + stream_name.substr(path.size() + 1);
      // First, check whether the specified "streamName" exists as a local file:
      Boolean fileExists = (access(fullName.c_str(), R_OK) == 0);

      // Next, check whether we already have a "ServerMediaSession" for this file:
      ServerMediaSession* sms = RTSPServer::lookupServerMediaSession(fullName.c_str());
      ...
      if (!fileExists) {
        ...
        if (sms == NULL) {
          sms = createNewSMS(envir(), streamName, fullName.c_str()); 
...
::addStreamPath(const std::string &path, const std::string &location)
{
  std::string loc = location;
  if (loc.back() != '/')
    loc.push_back('/');
  fStreamDirMap.emplace(path, loc);
}
...
#define NEW_SMS(description) do {\
    char const* descStr = description\
        ", streamed by live-streamer";\
    sms = ServerMediaSession::createNew(env, streamName, streamName, descStr);\
} while(0)

static ServerMediaSession* createNewSMS(UsageEnvironment& env,
					char const* streamName, char const* fileName) {
